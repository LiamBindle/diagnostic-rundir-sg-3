...
# Modify restart file
if len(sys.argv) != 2:
    raise ValueError("You didn't give the timestamp!")
...
da = xr.open_dataset(f'OutputDir/GCHP.SpeciesConc.{timestamp}.nc4')['SpeciesConc_Rn222']
ds_wind = xr.open_dataset(f'OutputDir/GCHP.StateMet_avg.{timestamp}.nc4')
u_wind = ds_wind['Met_U']
v_wind = ds_wind['Met_V']

ax = plt.subplot(gs1[0],  projection=projection)
ax.set_global()
...
for face in range(6):
    ...
    data = da.isel(time=0, nf=face, lev=18).squeeze()
    sg.plot.plot_pcolomesh(figax, xe, ye, data, vmin=0, vmax=3e-20, cmap=plt.cm.Reds)
    u = u_wind.isel(time=0, nf=face, lev=18).squeeze()
    v = v_wind.isel(time=0, nf=face, lev=18).squeeze()
...
plt.savefig(f'frames/{timestamp}.png', dpi=300)
